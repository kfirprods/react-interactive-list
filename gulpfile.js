...
var cssSrcPaths = './src/**/*.css'

gulp.task('clean-lib', function(cb) {
    del(jsLibPaths, cb)
})

gulp.task('move-css', ['clean-lib'], function() {
    return gulp.src(cssSrcPaths).pipe(gulp.dest('./lib'))
})

gulp.task('transpile-js', ['move-css'], function() {
    return gulp.src(jsSrcPaths)
        .pipe(plumber())
        .pipe(react({presets: ['es2015', 'react']}))
        .pipe(gulp.dest('./lib'))
})
